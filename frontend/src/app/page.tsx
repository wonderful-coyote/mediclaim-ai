...
export default function EHRTerminal() {
  ...
  const [consultantDrafts, setConsultantDrafts] = useState<
  ...
  const activeClaim = currentDoc
    ? reviewQueue
        ...
        .reverse()
        .find(
          (q) => q.doctorName === currentDoc.name && q.status !== "DISPATCHED",
        )
    : null;
  ...
  const isNotesValid =
    clinicalNotes.trim().length >= 10 && hasComplaint && hasDiagnosis;

  // 🌟 DB FETCH: Polling Patient Balance
  useEffect(() => {
  ...
  // 🌟 NEW DB FETCH: Polling Clinical Queue
  useEffect(() => {
    setIsMounted(true);
  ...
  const resubmitOrderWithUpdates = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        // 🌟 TYPE ERROR FIX: Ensure status and senderRole are cast to literal types
  ...
  const handleFinalDispatch = () => {
    const finalAmountToDeduct =
      auditResult?.deducted ?? activeClaim?.deductedAmount ?? 0;
    ...
    const procedureName =
      selectedTest?.name || activeClaim?.testName || "Medical Procedure";
    ...
    const rData: ReceiptData = {
      ...
      resolvedBy: activeClaim?.resolvedBy || "AI Auto-Approved",
    };
    ...
    if (activeClaim) {
      ...
      saveToDatabase(
        activeClaim.id,
        activeClaim.doctorName,
        activeClaim.testName,
        activeClaim.aiScore,
        "DISPATCHED",
        activeClaim.resolvedBy || "Peer Review",
        finalAmountToDeduct,
        finalPaycode,
        activeClaim.clinicalIndication || "",
      );
      ...
      saveToDatabase(
        ...
        finalAmountToDeduct,
        finalPaycode,
        auditResult.clinical_indication || "",
      );
    }
  ...
  const handleConsultantAction = (
    ...
    if (action === "COMMENT" || action === "NEEDS_INFO") {
      ...
      const updatedClaim: QueuedClaim = {
        ...
        messages: [...claim.messages, newMessage],
      };
      ...
      const updatedClaim: QueuedClaim = {
        ...
        resolvedBy: currentDoc.name,
        messages: draft ? [...claim.messages, newMessage] : claim.messages,
      };
      ...
      if (action === "REJECTED") {
        saveToDatabase(
          updatedClaim.id,
          updatedClaim.doctorName,
          updatedClaim.testName,
          updatedClaim.aiScore,
          "REJECTED",
          currentDoc.name,
          0,
          undefined,
          updatedClaim.clinicalIndication || "",
        );
      }
    }
      ...
      <main className="min-h-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 p-10 rounded-3xl shadow-2xl">
          ...
          <p className="text-sm text-slate-400 mb-8 text-center">
            Authenticate into the OpenMRS & Interswitch Secure Network.
          </p>
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col relative">
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs font-mono text-slate-800 bg-slate-100 px-2 py-1 rounded">
              ...
              <div className="space-y-3 border-b border-slate-100 pb-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-xs font-bold text-slate-800">
                    {receiptData.patientName} ({receiptData.patientId})
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs font-bold text-slate-500">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">
                    {receiptData.resolvedBy}
                  </span>
                </div>
              </div>

              {/* ===== UPDATED FINANCIALS SECTION ===== */}
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                  ...
                  <span className="text-sm font-black text-slate-800">
                    ₦{receiptData.totalCost.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-black text-blue-600">
                    ₦{receiptData.hmoAmount.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-black text-amber-600">
                    ₦{receiptData.patientCopay.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="border-t border-slate-200 pt-2">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-black text-slate-600 uppercase">
                      Charged to Smart Card
                    </span>
                    <span className="text-lg font-black text-emerald-600">
                      ₦{receiptData.amountDeducted.toLocaleString()}
                    </span>
                  </div>

                  {/* If card couldn't fully cover copay, show outstanding balance */}
                  {receiptData.amountDeducted < receiptData.patientCopay && (
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <span className="text-sm font-black text-rose-600">
                        ₦
                        {(
                          receiptData.patientCopay - receiptData.amountDeducted
                        ).toLocaleString()}
                      </span>
                    </div>
                  )}
                ...
                {/* Paycode Section — only shown when there's an outstanding balance */}
                {receiptData.paycode && (
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] font-black text-rose-600 uppercase tracking-widest block mb-1">
                          Balance Due — Patient Paycode
                        </span>
                        <span className="text-base font-mono font-black tracking-widest text-slate-800 bg-white px-3 py-1.5 rounded-lg border border-rose-200 shadow-sm">
                          {receiptData.paycode}
                        </span>
                      </div>
                      <div className="text-right mt-1">
                        ...
                        <span className="text-sm font-black text-rose-600">
                          ₦
                          {(
                            receiptData.patientCopay -
                            receiptData.amountDeducted
                          ).toLocaleString()}
                        </span>
                      </div>
                    </div>
                    <p className="text-[10px] text-rose-400 font-bold mt-2">
                      Direct patient to Cashier or POS terminal with this code.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-200 space-y-3">
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-white border border-slate-300 text-slate-700 py-3 rounded-xl font-bold text-xs uppercase tracking-wide hover:bg-slate-100 transition-colors shadow-sm"
                >
                  🖨️ Print
                </button>
                <button
              ...
              <button
                ...
                className="w-full bg-slate-900 text-white py-3.5 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-black transition-colors shadow-md"
              >
                Done & Close
              </button>
            </div>
      ...
      <div className="w-full bg-slate-900 text-white p-3 flex justify-between items-center px-6 shadow-md z-40 relative">
        <div className="flex items-center gap-3">
          ...
          <span className="font-black tracking-widest text-xs uppercase text-slate-300 hidden md:block">
            MediClaim Network
          </span>
        </div>
        ...
        <div className="flex items-center gap-4 bg-slate-800 rounded-lg p-1.5 border border-slate-700 pr-4">
          <div
            className={`text-white w-8 h-8 rounded-md flex items-center justify-center font-bold text-[10px] uppercase ${currentDoc.rank === "Cons" ? "bg-amber-600" : "bg-blue-600"}`}
          >
            {currentDoc.rank}
          </div>
      ...
      {/* ========================================== */}
      {/* SCREEN 1: JUNIOR DOCTOR TERMINAL           */}
      {/* ========================================== */}
        ...
        <main className="flex-1 p-6 md:p-12 flex items-center justify-center">
          <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            <div className="space-y-6">
                ...
                <div className="bg-linear-to-br from-slate-800 to-slate-950 p-8 rounded-2xl text-white shadow-xl relative min-h-55 flex flex-col justify-between">
                  ...
                  <div className="space-y-4">
                    <div className="w-full">
                      ...
                      <p className="text-2xl sm:text-3xl font-black text-emerald-400 leading-none truncate">
              ...
              {hoNotificationCount > 0 && (
                <div className="bg-amber-100 border border-amber-200 rounded-xl p-4 text-center animate-pulse shadow-sm">
                  <p className="text-amber-800 font-black text-xs uppercase">
                    Action Required
                  </p>
                  <p className="text-amber-700 text-[10px]">
                    Your pending claim has been updated.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="md:col-span-2 bg-white p-10 rounded-3xl shadow-2xl border-t-8 border-blue-600">
              {auditResult &&
                !isReviewing &&
                !["APPROVED", "REJECTED", "NEEDS_INFO", "PENDING"].includes(
                  activeClaim?.status || "",
                ) && (
                  <button
                    onClick={() => setAuditResult(null)}
                    className="mb-6 flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-blue-600 transition-colors"
                  >
              ...
              <h2 className="text-2xl font-black text-slate-800 mb-8 flex justify-between items-center tracking-tight">
                EHR Clinical Order
                <span className="text-[9px] bg-slate-100 px-2 py-1 rounded text-slate-500 font-bold uppercase">
                  {status}
                </span>
              </h2>
              ...
              {!auditResult && !activeClaim ? (
                <div className="animate-in fade-in">
                    ...
                    {selectedTest && (
                      <div className="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-100">
                        ...
                        <span className="text-xs text-slate-500">
                          ...
                          | Co-pay (20%): ₦
                          {(selectedTest.cost * 0.2).toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mb-6">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">
                      Clinical Justification (For AI Audit)
                    </label>
                    <textarea
                      value={clinicalNotes}
                    ...
                    {!isNotesValid && clinicalNotes.trim().length > 0 && (
                      <p className="text-rose-500 text-[10px] font-bold mt-2">
                        ⚠️ Notes must include a Presenting Complaint (e.g.,
                        &ldquo;C/O:&rdquo;) AND a Diagnosis/Assessment (e.g.,
                        &ldquo;Dx:&rdquo;).
                      </p>
                    )}
                  </div>
                  ...
                  >
                    {loading
                      ? "PROCESSING..."
                      : !isNotesValid
                        ? "ENTER P/C & DX TO UNLOCK"
                        : balance > 0
                          ? "AUTHORIZE & AUDIT"
                          : "AUTHORIZE & AUDIT (WALLET EMPTY)"}
                  </button>
                </div>
              ) : (
                <div className="animate-in slide-in-from-bottom-4">
                  <div
                    className={`p-6 rounded-2xl border-2 mb-4 ${auditResult?.status === "AUTHORIZED" ? "bg-emerald-50 border-emerald-100" : "bg-amber-50 border-amber-100"}`}
                  >
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <h3
                          className={`text-lg font-black uppercase ${auditResult?.status === "AUTHORIZED" ? "text-emerald-800" : "text-amber-800"}`}
                        >
                    ...
                    {(auditResult?.reasoning || activeClaim?.aiReasoning) && (
                      <div className="mb-4 p-3 bg-blue-50/50 rounded-lg border border-blue-100 text-xs text-blue-800 leading-relaxed">
                        <span className="font-black mr-2 tracking-tight">
                          🧠 AI INSIGHT:
                        </span>
                        {auditResult?.reasoning || activeClaim?.aiReasoning}
                      </div>
                    ...
                    <div className="text-[11px] font-bold text-slate-600 uppercase space-y-2 mt-4 border-t border-slate-100 pt-3">
                      <div className="flex justify-between items-center">
                        <span>Pre-Auth Card Hold:</span>
                        <span className="text-slate-800">
                          ₦{displayHoldAmount.toLocaleString()}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between items-center border-t border-dashed border-slate-200 pt-2 mt-2">
                        ...
                        <span
                          className={`px-3 py-1 rounded-md text-[10px] font-black tracking-widest ${
                            (auditResult?.payout_tier ||
                              activeClaim?.payout_tier) === "Instant Payout"
                              ? "bg-emerald-500 text-white shadow-md shadow-emerald-500/20"
                              : (auditResult?.payout_tier ||
                                    activeClaim?.payout_tier) ===
                                  "72-Hour HMO Audit"
                  ...
                  <div className="space-y-4">
                    {(auditResult?.audit_score || 0) >= 0.9 &&
                    auditResult?.status === "AUTHORIZED" ? (
                      <button
                        onClick={handleFinalDispatch}
                        className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black uppercase shadow-lg hover:bg-emerald-700"
                      >
                        DISPATCH TO{" "}
                        {selectedTest?.dept?.toUpperCase() || "UNIT"}
                      </button>
                    ) : (
                      <div className="space-y-4">
                        {activeClaim?.status === "PENDING" || isReviewing ? (
                          <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl text-center animate-in fade-in mt-4 shadow-sm">
                            <div className="w-12 h-12 bg-amber-100 text-amber-500 rounded-full flex items-center justify-center mx-auto mb-4">
                              <span className="font-black text-xl animate-pulse">
                                ⏳
                              </span>
                            </div>
                            <p className="text-amber-800 font-black text-sm uppercase mb-2">
                            ...
                            <p className="text-amber-700 text-xs font-medium mb-2">
                              ...
                              <strong className="font-black">
                                {activeClaim?.reviewerNames.join(" & ") ||
                                  "Consultants"}
                              </strong>
                            </p>
                          </div>
                          ...
                          <div className="space-y-4 animate-in fade-in">
                            <div className="bg-emerald-100 p-4 rounded-xl border border-emerald-200 text-center">
                              <p className="text-emerald-800 font-black text-sm uppercase mb-1">
                                ✅ Peer Review Approved
                              </p>
                              <p className="text-emerald-700 text-xs">
                                Authorized by:{" "}
                                <strong>{activeClaim.resolvedBy}</strong>
                              </p>
                            </div>
                            <button
                              onClick={handleFinalDispatch}
                          ...
                          <div className="bg-rose-100 p-4 rounded-xl border border-rose-200 text-center animate-in fade-in">
                            <p className="text-rose-800 font-black text-sm uppercase mb-2">
                              ❌ Claim Rejected
                            </p>
                            <p className="text-rose-700 text-xs mb-4">
                              Denied by:{" "}
                              <strong>{activeClaim.resolvedBy}</strong>. Funds
                              were not deducted.
                            </p>
                            <button
                              onClick={() => {
                                setReviewQueue((prev) => prev.filter((q) => q.id !== activeClaim.id));
                                deleteClaimFromDB(activeClaim.id);
                                setAuditResult(null);
                                setLoadedClaimId(null);
                                setClinicalNotes("");
                              }}
                          ...
                          <div className="space-y-4 animate-in fade-in">
                            <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">
                                Update Clinical Notes (AI Will Re-Grade)
                              </p>
                              <textarea
                                value={clinicalNotes}
                                onChange={(e) =>
                                ...
                                className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all min-h-25"
                              />
                            </div>

                            <div className="border border-blue-200 bg-blue-50 rounded-2xl flex flex-col overflow-hidden shadow-sm">
                              <div className="bg-blue-100/50 border-b border-blue-200 p-3 px-4">
                              ...
                              <div className="p-4 flex flex-col-reverse gap-3 max-h-48 overflow-y-auto bg-slate-50/30">
                                {[...activeClaim.messages]
                                  ...
                                  .map((msg, idx) => (
                                    <div
                                      key={idx}
                                      className={`p-3 rounded-xl text-sm shadow-sm max-w-[85%] ${msg.senderRole === "Consultant" ? "bg-white border border-blue-100 mr-auto" : "bg-blue-600 text-white ml-auto"}`}
                                    >
                                      <p
                                        className={`text-[9px] font-bold mb-1 ${msg.senderRole === "Consultant" ? "text-blue-400" : "text-blue-200"}`}
                                      >
                              ...
                              <div className="p-4 bg-white border-t border-blue-100">
                                ...
                                <textarea
                                  value={hoReplyText}
                                  onChange={(e) =>
                                    setHoReplyText(e.target.value)
                                  }
                                  placeholder="Type your response to the consultant's query here..."
                                  className="w-full p-3 rounded-xl border border-slate-200 bg-slate-50 text-sm mb-3 outline-none focus:ring-2 focus:ring-blue-500 min-h-20"
                                />

                                <button
                                  onClick={resubmitOrderWithUpdates}
                                  disabled={
                                    loading || hoReplyText.trim().length === 0
                                  }
                                  className={`w-full py-3.5 rounded-xl font-black uppercase text-sm shadow-md transition-all flex justify-center items-center gap-2 ${
                                    loading || hoReplyText.trim().length === 0
                                ...
                                >
                                  {loading
                                    ? "RE-AUDITING..."
                                    : hoReplyText.trim().length === 0
                                      ? "TYPE A REPLY TO CONTINUE"
                                      : "Update Notes & Send Reply"}
                                </button>
                              </div>
                            </div>
                          </div>
                          ...
                          <>
                            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">
                                ...
                                {selectedReviewers.length}/{getRequiredCount()})
                              </p>
                              <div className="grid grid-cols-1 gap-2">
                                {staffDirectory
                                  .filter(
                                    (doc) =>
                                      ["Cons", "SR"].includes(doc.rank) &&
                                      doc.id !== currentDoc.id,
                                  )
                                  .map((doc) => (
                                    <button
                                      key={doc.id}
                                      onClick={() => toggleReviewer(doc)}
                                      className={`p-3 rounded-xl border text-left flex justify-between items-center transition-all ${selectedReviewers.find((r) => r.id === doc.id) ? "border-blue-600 bg-blue-50" : "bg-white border-slate-100 hover:border-blue-200"}`}
                                    >
                                      <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 bg-slate-200 rounded-full shrink-0 flex items-center justify-center text-[9px] font-bold uppercase">
                                          {doc.rank}
                                        </div>
                                        <div>
                            ...
                            >
                              {selectedReviewers.length < getRequiredCount()
                                ? `SELECT ${getRequiredCount() - selectedReviewers.length} MORE REVIEWER(S)`
                                : "FORWARD FOR REVIEW"}
                            </button>
                          </>
                        )}
        ...
        <main className="flex-1 p-6 md:p-12 bg-slate-100">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-3xl font-black tracking-tight text-slate-800 mb-2">
              Senior Consultant Dashboard
            </h1>
            <p className="text-sm text-slate-500 mb-8 font-medium">
              Review, Query, and Authorize AI-flagged clinical orders.
            </p>
            ...
            {myConsultantQueue.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-dashed border-slate-300">
                ...
                <p className="text-slate-400 text-xs mt-2">
                  No Junior Doctors have assigned clinical requests to you.
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                {[...myConsultantQueue].reverse().map((claim) => (
                  ...
                  >
                    <div
                      className={`p-4 flex justify-between items-center ${claim.status === "PENDING" ? "bg-amber-50 border-b border-amber-100" : claim.status === "NEEDS_INFO" ? "bg-blue-50 border-b border-blue-100" : claim.status === "APPROVED" ? "bg-emerald-50 border-b border-emerald-100" : claim.status === "DISPATCHED" ? "bg-slate-100 border-b border-slate-200" : "bg-rose-50 border-b border-rose-100"}`}
                    >
                      <div className="flex items-center gap-3">
                        <span
                          className={`px-2 py-1 rounded text-[10px] font-black uppercase text-white ${claim.status === "PENDING" ? "bg-amber-500" : claim.status === "NEEDS_INFO" ? "bg-blue-500" : claim.status === "APPROVED" ? "bg-emerald-500" : claim.status === "DISPATCHED" ? "bg-slate-500" : "bg-rose-500"}`}
                        >
                        ...
                        {(claim.status === "APPROVED" ||
                          ...
                          claim.resolvedBy && (
                            <span className="text-[10px] font-bold text-slate-500 bg-white/50 px-2 py-1 rounded-full border border-white">
                              By {claim.resolvedBy}
                            </span>
                          )}
                      </div>
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-sm italic text-slate-600 mb-4 shadow-inner">
                          ...
                          <div className="mt-4 border-t border-slate-100 pt-4">
                            ...
                            <div className="flex flex-col-reverse gap-2 max-h-40 overflow-y-auto pr-1">
                              {[...claim.messages].reverse().map((msg, idx) => {
                                const isMe =
                                  msg.senderName === currentDoc?.name;
                                const isOtherCons =
                                  msg.senderRole === "Consultant" && !isMe;
                                  ...
                                  <div
                                    ...
                                    className={`p-2 rounded-lg text-sm shadow-sm ${
                                      isMe
                                        ...
                                        : isOtherCons
                                          ? "bg-indigo-50 border border-indigo-100 text-indigo-900 mr-8"
                      ...
                      <div className="space-y-4 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-200 mb-4">
                            ...
                            <span
                              className={`text-2xl font-black ${claim.aiScore >= 0.9 ? "text-emerald-500" : claim.aiScore >= 0.75 ? "text-amber-500" : "text-rose-500"}`}
                            >
                          ...
                          <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                            ...
                            {claim.suggestions && claim.suggestions.length > 0 && (
                              <div className="mt-3 border-t border-blue-200/60 pt-3">
                                <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mb-1 flex items-center gap-1">
                                  💡 Actionable Suggestions
                                </p>
                                <ul className="list-disc pl-4 space-y-1">
                                  {claim.suggestions.map((sug: string, i: number) => (
                                    <li key={i} className="text-[10px] text-blue-800 font-medium leading-relaxed">
                                      {sug}
                                    </li>
                                  ))}
                                </ul>
                        ...
                        <div className="mt-4">
                          <textarea
                            value={consultantDrafts[claim.id] || ""}
                            onChange={(e) =>
                              setConsultantDrafts({
                                ...consultantDrafts,
                            ...
                            placeholder={
                              claim.status === "PENDING"
                                ? "Type a query or note..."
                                : "Add a post-review audit comment..."
                            }
                            className="w-full p-3 rounded-xl border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-blue-500 mb-2"
                          />
                          ...
                          {claim.status === "PENDING" ? (
                            <div className="flex gap-2 mt-2">
                              <button
                                onClick={() =>
                                  handleConsultantAction(claim.id, "NEEDS_INFO")
                                }
                                disabled={!consultantDrafts[claim.id]}
                                className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-black uppercase text-[10px] hover:bg-blue-700 transition-colors disabled:opacity-50 shadow-md"
                              >
                                💬 Send Query
                              </button>
                              <button
                                onClick={() =>
                                  handleConsultantAction(claim.id, "APPROVED")
                                }
                                className="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] hover:bg-emerald-700 transition-colors shadow-md"
                              >
                                ✅ Authorize
                              </button>
                              <button
                                onClick={() =>
                                  handleConsultantAction(claim.id, "REJECTED")
                                }
                                className="flex-1 bg-rose-100 text-rose-700 py-3 rounded-xl font-black uppercase text-[10px] hover:bg-rose-200 transition-colors shadow-md"
                              >
                                ❌ Reject
                              </button>
                            </div>
                            ...
                            <button
                              onClick={() =>
                                handleConsultantAction(claim.id, "COMMENT")
                              }
                              disabled={!consultantDrafts[claim.id]}
                              className="w-full bg-slate-800 text-white py-3 rounded-xl font-black uppercase text-xs hover:bg-slate-700 transition-colors disabled:opacity-50 shadow-md"
                            >
                              Leave Audit Comment
                            </button>
                          )}
