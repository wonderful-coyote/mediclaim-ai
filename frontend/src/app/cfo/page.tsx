...
const getClaimTone = (
  ...
  const outOfPocketTotal = Math.max(
    (claim.total_cost || 0) - (claim.hmo_payout || 0),
    0,
  );
  const outstanding = Math.max(
    outOfPocketTotal - (claim.deducted_amount || 0),
    0,
  );

  if (settlement.includes("REJECTED") || claim.status === "REJECTED")
    return "rose";
  if (settlement.startsWith("PATIENT_RESPONSIBLE"))
    return outstanding > 0 ? "amber" : "indigo";
...
const getClaimLabel = (claim: Claim): string => {
  ...
  if (settlement.includes("REJECTED") || claim.status === "REJECTED")
    return "Rejected";
  if (settlement.startsWith("PATIENT_RESPONSIBLE"))
    return outstanding > 0 ? "Self-Pay Pending" : "Self-Pay Cleared";
  if (settlement === "FULLY_SETTLED") return "Fully Settled";
  if (settlement === "HMO_APPROVED_PENDING_PT")
    return outstanding > 0 ? "Co-Pay Pending" : "Awaiting HMO Release";
...
export default function AdminDashboard() {
  ...
  const handleClearPOS = async (claimId: string): Promise<void> => {
    try {
      await fetch(`${API_BASE_URL}/api/v1/admin/clear-pos-payment/${claimId}`, {
        method: "POST",
      });
  ...
  const handleRequestWalletDeduction = async (
    claimId: string,
  ): Promise<void> => {
    setProcessingId(`req-${claimId}`);
    try {
      await fetch(
        `${API_BASE_URL}/api/v1/admin/request-wallet-deduction/${claimId}`,
        {
          method: "POST",
        },
      );
      ...
      console.error("Failed to request wallet deduction:", error);
    } finally {
  ...
  const filteredClaims = useMemo(() => {
    return claims.filter((claim: Claim) => {
      ...
      const isRejected =
        settlement.includes("REJECTED") || claim.status === "REJECTED";
      const isSelfPay =
        settlement.startsWith("PATIENT_RESPONSIBLE") ||
        ((claim.hmo_payout || 0) <= 0 &&
          (claim.total_cost || 0) > 0 &&
          !isRejected);
  ...
  const fraudPrevented = useMemo(
    () =>
      claims
        .filter((c: Claim) =>
          String(c.settlement_status || "").includes("REJECTED"),
        )
        .reduce((acc: number, curr: Claim) => acc + (curr.total_cost || 0), 0),
    [claims],
  );
  ...
  const stpCount = claims.filter(
    (c: Claim) =>
      (c.hmo_payout || 0) > 0 &&
      ["INSTANT_SETTLED", "FULLY_SETTLED", "HMO_APPROVED_PENDING_PT"].includes(
        String(c.settlement_status || "").toUpperCase(),
      ),
  ).length;
  const aiAutomationRate =
    claims.length > 0 ? Math.round((stpCount / claims.length) * 100) : 0;
      ...
      <main className="min-h-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 p-10 rounded-3xl shadow-2xl">
          ...
          <h1 className="text-2xl font-black text-white tracking-tight mb-2 text-center">
            MediClaim Enterprise
          </h1>
          <p className="text-sm text-slate-400 mb-8 text-center">
            Authenticate to access the Revenue & Audit Terminal.
          </p>
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              ...
              <div className="w-full p-4 rounded-xl bg-slate-800 border border-slate-700 text-white font-bold">
                {selectedAdmin} (Chief Financial Officer)
              </div>
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">
              ...
              <input
                ...
                placeholder="PIN (123)"
                className={`w-full p-4 rounded-xl bg-slate-800 border text-white outline-none focus:border-indigo-500 tracking-widest ${loginError ? "border-rose-500" : "border-slate-700"}`}
              />
              {loginError && (
          ...
          <div className="bg-white rounded-3xl max-w-5xl w-full shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="p-6 border-b border-slate-200 flex justify-between items-start gap-4">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                  Stored CFO Claim Record
                </p>
                <h3 className="text-2xl font-black text-slate-800 tracking-tight">
                  {selectedClaim.procedure_name}
                </h3>
                <p className="text-xs text-slate-500 mt-1">
                  {selectedClaim.patient_id} • {selectedClaim.doctor_name} •{" "}
            ...
            <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="font-black text-slate-800 wrap-break-word whitespace-normal">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 md:col-span-2">
                ...
                <p className="text-slate-700 leading-relaxed wrap-break-word whitespace-pre-wrap">
                  {selectedClaim.notes || "No notes stored."}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 md:col-span-2">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between gap-4">
                    ...
                    <span className="font-black text-slate-800 break-all text-right">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 md:col-span-2">
                ...
                {parseJsonArray<string>(selectedClaim.suggestions_json).length >
                0 ? (
                  <ul className="list-disc pl-5 space-y-1 text-slate-700">
                    {parseJsonArray<string>(selectedClaim.suggestions_json).map(
                      (s, i) => (
                        <li key={i}>{s}</li>
                      ),
                    )}
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 md:col-span-2">
                ...
                {parseJsonArray<Message>(selectedClaim.messages_json).length >
                0 ? (
                  <div className="space-y-2">
                    {parseJsonArray<Message>(selectedClaim.messages_json).map(
                      (msg, idx) => (
                        <div
                          key={idx}
                        ...
                        >
                          ...
                          <p className="text-slate-700 wrap-break-word whitespace-pre-wrap">
                            {msg.text || ""}
                          </p>
                        </div>
                      ),
      ...
      <div className="w-full bg-slate-900 text-white p-3 flex justify-between items-center px-6 shadow-md z-40 sticky top-0">
        <div className="flex items-center gap-3">
          ...
          <span className="font-black tracking-tight text-lg text-slate-100 hidden md:block">
            MediClaim{" "}
            <span className="font-medium text-slate-400">Enterprise</span>
          </span>
        </div>
      ...
      <main className="flex-1 p-6 md:p-12 max-w-375 w-full mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">
            Revenue Cycle & AI Audit Dashboard
          </h1>
          <p className="text-sm text-slate-500 font-medium mt-1">
            Real-time adjudication, self-pay tracking, fraud prevention, and
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 mb-10">
          <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl">
            <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">
              Approved HMO Receivables
            </p>
            <h2 className="text-2xl lg:text-3xl font-black tracking-tighter mt-1">
              {formatNaira(wallet.pending_escrow)}
            </h2>
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-3">
              Expected HMO remittance
            </p>
          </div>
          <div className="bg-white p-6 rounded-3xl shadow-md border border-slate-200">
            ...
            <h2 className="text-2xl lg:text-3xl font-black text-slate-800 tracking-tighter mt-1">
              {formatNaira(wallet.available_balance)}
            </h2>
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-3">
              Wallet / POS cash realized
            </p>
          </div>
          <div className="bg-amber-50 p-6 rounded-3xl shadow-md border border-amber-200">
            <p className="text-[9px] font-black text-amber-700 uppercase tracking-widest mb-1">
              Self-Pay Outstanding
            </p>
            <h2 className="text-2xl lg:text-3xl font-black text-rose-600 tracking-tighter mt-1">
              {formatNaira(selfPayOutstanding)}
            </h2>
            <p className="text-[9px] text-amber-700/70 font-bold uppercase tracking-widest mt-3">
              Still due from patients
            </p>
          </div>
          <div className="bg-rose-50 p-6 rounded-3xl shadow-md border border-rose-200">
            <p className="text-[9px] font-black text-rose-600 uppercase tracking-widest mb-1">
              Fraud / Waste Prevented
            </p>
            <h2 className="text-2xl lg:text-3xl font-black text-rose-700 tracking-tighter mt-1">
              {formatNaira(fraudPrevented)}
            </h2>
            <p className="text-[9px] text-rose-600/70 font-bold uppercase tracking-widest mt-3">
              Auto / policy rejects
            </p>
          </div>
          <div className="bg-emerald-50 p-6 rounded-3xl shadow-md border border-emerald-200">
            <p className="text-[9px] font-black text-emerald-700 uppercase tracking-widest mb-1">
              AI Automation Efficiency
            </p>
            <h2 className="text-2xl lg:text-3xl font-black text-emerald-600 tracking-tighter mt-1">
              {aiAutomationRate}%
            </h2>
            <p className="text-[9px] text-emerald-700/70 font-bold uppercase tracking-widest mt-3">
          ...
          <div className="px-8 py-6 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
              <h2 className="text-lg font-black text-slate-800 tracking-tight">
                Master Claims & Audit Ledger
              </h2>
              <p className="text-xs text-slate-500 mt-1">
                HMO-funded, self-pay, rejected, and cashier-clearance flows in
                one place.
              </p>
            </div>
            <div className="flex flex-wrap bg-slate-200/50 p-1 rounded-xl gap-1">
              {(
                ["ALL", "HMO", "SELF_PAY", "POS", "REJECTED"] as FilterType[]
              ).map((filter) => (
                <button
                  key={filter}
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredClaims.length === 0 ? (
                    ...
                    filteredClaims.map((claim: Claim) => {
                      ...
                      const isSelfPay = String(claim.settlement_status || "")
                        .toUpperCase()
                        .startsWith("PATIENT_RESPONSIBLE");
                        ...
                        >
                          <td className="p-4 pl-6 align-top">
                            <p className="text-sm font-black text-slate-800">
                              ...
                              <span className="text-indigo-600 font-bold text-xs ml-1">
                                for {claim.patient_id}
                              </span>
                            </p>
                            ...
                            <p className="text-[9px] text-slate-400 uppercase tracking-widest font-mono mt-1.5 mb-2">
                              {claim.claim_id}
                            </p>
                            <p className="text-[11px] text-slate-500 leading-relaxed max-w-sm">
                              {claim.reasoning || "No AI reasoning stored."}
                            </p>
                          </td>
                          <td className="p-4 align-top">
                            <span
                              className={`px-2 py-1 text-[9px] font-black uppercase tracking-widest rounded-lg border mb-2 inline-block ${badgeClass[tone]}`}
                            >
                              {label}
                            </span>
                          ...
                          <td className="p-4 align-top pt-5 text-sm font-bold text-slate-800">
                            {formatNaira(claim.total_cost || 0)}
                          </td>
                          <td className="p-4 align-top pt-5">
                            <span
                              className={`text-sm font-black ${isRejected || isSelfPay ? "text-slate-400" : "text-indigo-600"}`}
                            >
                              {formatNaira(claim.hmo_payout || 0)}
                            </span>
                            <br />
                            <span className="text-[8px] uppercase tracking-widest font-bold mt-1 block text-slate-400">
                              {isSelfPay
                                ? "NO HMO FUNDING"
                                : isRejected
                                  ? "DENIED"
                          ...
                          <td className="p-4 align-top pt-5">
                            <span
                              className={`text-sm font-black ${coPayOwed > 0 ? "text-rose-600" : "text-emerald-600"}`}
                            >
                          ...
                          <td className="p-4 pr-6 text-right align-top pt-4">
                            <div className="flex flex-col gap-2 items-end">
                              <button
                                ...
                                className="px-4 py-2 bg-slate-900 hover:bg-black text-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-sm transition-all w-full"
                              >
                                View
                              </button>

                              {claim.paycode && coPayOwed > 0 && (
                                <div className="flex flex-col gap-2 w-full mt-2 border-t border-slate-100 pt-2">
                                  {claim.wallet_request_status ===
                                  "REQUESTED" ? (
                                    <div className="w-full text-center p-2 bg-amber-50 rounded-lg border border-amber-200">
                                      <span className="text-[9px] font-black uppercase tracking-widest text-amber-600 flex items-center justify-center gap-2">
                                    ...
                                    <div className="flex flex-col gap-2 w-full">
                                      <div className="w-full text-center p-1.5 bg-rose-50 rounded-lg border border-rose-200">
                                      ...
                                      <button
                                        ...
                                        className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-400 text-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-md transition-all flex flex-col items-center justify-center"
                                      >
                                        <span>
                                          {processingId ===
                                          `req-${claim.claim_id}`
                                    ...
                                    <button
                                      ...
                                      disabled={
                                        processingId === `req-${claim.claim_id}`
                                      }
                                      className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-400 text-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-md transition-all flex flex-col items-center justify-center"
                                    >
                                      <span>
                                  ...
                                  <button
                                    onClick={() =>
                                      handleClearPOS(claim.claim_id)
                                    }
                                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-md transition-all flex flex-col items-center justify-center"
                                  >
                                    <span>Clear POS / Cash</span>
                                    <span className="font-mono mt-0.5 text-xs">
                                      {claim.paycode}
                                    </span>
                                  </button>
                                </div>
